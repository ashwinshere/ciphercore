// ============================================================================
// VERTEX / CIPHERCORE — src/data/gisData.js
//
...
// Supports GeoJSON conversion, drone imagery anchors, and LiDAR survey grids.
// ============================================================================
...
export const gisData = {
  ...
  campus: {
    ...
    utmProjection: "UTM Zone 44N (EPSG:32644)",
    localCoordinateSystem: "Local Cartesian Metric (XYZ)",
    surveyNumber: "SF-248/1A",
    totalAreaAcres: 40.5,
    dataConfidence: "Verified Cadastral Baseline",
    statusNote: "Georeferenced WGS84 Spatial Framework",
    mapDefaultZoom: 18,
    bounds: [
  ...
  pilotBuilding: {
    ...
    unitsPerFloor: 10,
    orientationDeg: 32,
    dataConfidence: "High Accuracy 3D Twin",
    status: "Active 3D Cadastral Digital Twin",
...
/**
 * Convert property list to a standard GeoJSON FeatureCollection
 */
export function propertiesToGeoJSON(propertiesList = []) {
  return {
    ...
    features: propertiesList.map((p) => {
      ...
      const coordinates = p.footprint
        ? [p.footprint.map(([lat, lng]) => [lng, lat])]
        : [];
      ...
      return {
        ...
        properties: {
          propertyId: `PROP-${p.blockNumber || '001'}`,
          ulpin: p.ulpin2D,
          name: p.name,
          ...
          floorsCount: p.floors,
          buildingHeightM: p.buildingHeightM,
          footprintAreaM2: p.footprintWidthM * p.footprintDepthM,
          status: p.prototypeStatus || "verified",
          confidence: p.geometryConfidence || "high",
          buildingIds: [p.id]
        }
      };
    })
 ...
 */
export function localToGeographic(localX, localZ, anchorLat = 10.757450, anchorLng = 78.652400, rotationDeg = 32) {
